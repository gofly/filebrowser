...
var httpClient = &http.Client{
	Timeout: 1 * time.Second, // 1 seconds
}

func getDownloadBaseURL(downloadBaseURLAPI string) (string, error) {
	resp, err := httpClient.Get(downloadBaseURLAPI)
	if err != nil {
		return "", err
	}
	defer func() {
...
func handleWithStaticData(w http.ResponseWriter, _ *http.Request, d *data, fSys fs.FS, file, contentType string) (int, error) {
	...
	downloadBaseURL := d.server.DownloadBaseURL
	if d.server.DownloadBaseURLAPI != "" {
		downloadBaseURL, err = getDownloadBaseURL(d.server.DownloadBaseURLAPI)
		if err != nil {
			return http.StatusInternalServerError, err
		}
	}
